...
/-! Residual structure after ordinary models. Source: Raven/features.py.
...
def baseComponents : List String := ["out['residual_mean_abs']", "out['residual_variance']", "out['residual_coupling']", "out['residual_persistence']"]
def horizonComponents : List String := ["out[f'residual_mean_{h}']", "out[f'residual_var_{h}']", "out[f'residual_velocity_{h}'].abs()"]
def expectedColumns (horizons : Nat) : Nat := 4 + 3 * horizons
