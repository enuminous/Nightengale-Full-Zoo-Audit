...
/-! Distributed partial observability. Source: Octopus/latent_state.py.
...
namespace Zoo.Octopus

def baseComponents : List String := ["graph_features['distributed_contradiction']", "graph_features['distributed_dispersion']", "graph_features['missing_fraction']"]
def horizonComponents : List String := ["graph_features[f'dc_mean_{h}']", "graph_features[f'dd_mean_{h}']", "graph_features[f'missing_mean_{h}']"]
def expectedColumns (horizons : Nat) : Nat := 3 + 3 * horizons
