...
/-! Motion and control hazards. Source: Shark/features.py.
...
def baseComponents : List String := ["out['kinematic_contradiction']", "out['control_oscillation']", "out['control_reversal_rate']", "out['boundary_pressure']", "out['obstacle_pressure']", "out['recovery_pressure']", "out['hazard_velocity'].clip(lower=0)", "out['hazard_acceleration'].clip(lower=0)", "out['recursive_hazard']"]
def horizonComponents : List String := ["out[f'contradiction_mean_{h}']", "out[f'oscillation_mean_{h}']", "out[f'boundary_mean_{h}']", "out[f'recovery_mean_{h}']"]
def expectedColumns (horizons : Nat) : Nat := 9 + 4 * horizons
