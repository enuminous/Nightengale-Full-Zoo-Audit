...
/-! Noise-regime evaluation. Source: Penguin/features.py.
...
namespace Zoo.Penguin

def baseComponents : List String := ["out['noise_variance']", "out['gradient_instability']", "out['cross_channel_disagreement']", "out['entropy_proxy']", "out['scale_drift']", "1.0 / (out['snr_proxy'] + EPS)"]
def horizonComponents : List String := ["out[f'noise_mean_{h}']", "1.0 / (out[f'snr_mean_{h}'] + EPS)", "out[f'entropy_mean_{h}']", "out[f'gradient_mean_{h}']"]
